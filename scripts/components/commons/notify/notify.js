 ...
 * Angular Notify
 * @author Michael
 * @copyright 2015.11.11 Michael
 */
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        // AMD
        define(['angularAMD', 'bootstrap'], factory);
    } else if (typeof exports === 'object') {
        // CommonJS
        factory(require('angular'), require('bootstrap'));
    } else {
        // Browser globals
        factory(window.angular, window.bootstrap);
    }
}(function (angularAMD) {
    'use strict';
    ...
    angular.module('commons.notify',[])

    .run(['$rootScope', 'vanNotify', function($rootScope, vanNotify){

        // append stylesheet to head
        var stylesheet = angular.element('link[href="public/scripts/components/commons/notify/notify.css"]');
        if(stylesheet.length == 0){
            angular.element('head').append('<link href="public/scripts/components/commons/notify/notify.css" rel="stylesheet">');
        }

        var notify = function(args, type, delay){
            var _args = {};

            if(args && angular.isString(args)){
                _args.message = args;
            }
            else if(args && angular.isObject(args)){
                _args = args;
            }
            else{
                return;
            ...
            if(delay && angular.isNumber(delay) && delay >= 100){
                _args.delay = delay;
            }

            vanNotify.call(vanNotify, _args, type);
        };

        $rootScope.notify = angular.extend(notify, vanNotify);
    }])

    .provider('vanNotify', function() {
    ...
    .run(["$templateCache", function($templateCache) {$templateCache.put("angular-ui-notification.html","<div class=\"ui-notification\"><h3 ng-show=\"title\" ng-bind-html=\"title\"></h3><div class=\"message\" ng-bind-html=\"message\"></div></div>");}]);
